...
class BoardViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if topic_pk is not None:
            queryset = queryset.filter(topic=topic_pk)
...
class ThreadViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        queryset = Thread.objects.with_post_counts_and_last_replied().order_by(
            "-is_sticky", "-last_replied"
        )
        ...
        if (
            ...
            and not self.request.user.is_administrator
        ):
            queryset = queryset.none()

        # users can only delete their own threads
        elif (
            self.action in ["destroy"]
            and not self.request.user.is_moderator
            and not self.request.user.is_administrator
        ):
...
class PostViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if username is not None:
            queryset = queryset.filter(author_username=username).order_by(
                "-date_created"
            )
